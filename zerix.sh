...
load_resources(){
  ...
  [[ -s "$TMP_DIR/resources/scripts/Zerix/main.css" ]] || die "Failed to download main.css"
  [[ -s "$TMP_DIR/resources/scripts/Zerix/theme.ts" ]] || die "Failed to download theme.ts"
  [[ -s "$TMP_DIR/resources/scripts/Zerix/assets/zerix-icon.svg" ]] || die "Failed to download zerix-icon.svg"
  [[ -s "$TMP_DIR/resources/scripts/Zerix/assets/theme.json" ]] || die "Failed to download theme.json"
...
build_panel(){
  ...
  if [[ -f "$PANEL/artisan" ]] && command -v php >/dev/null 2>&1; then
